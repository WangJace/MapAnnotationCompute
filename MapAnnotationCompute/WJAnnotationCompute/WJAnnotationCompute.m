...
+(NSMutableArray *)rangeMapDataWithArray:(NSArray *)array Distance:(CGFloat)distance
{
    ...
    //做两层遍历，当某个标注MAUserAnnotation被合并，则改标注的flag属性改为YES，之后的遍历就可跳过这个标注
    ...
    for (int i = 0; i < array.count; i++) {
        //数组中有可能存在当前位置（MAUserLocation）或者其他非WJAnnotationModel类的对象
        if ([[array objectAtIndex:i] isKindOfClass:[WJAnnotationModel class]]) {
            //当前作为距离判断基点的标注
            WJAnnotationModel *selectedAnnotation = [array objectAtIndex:i];
            //判断标注是否已经被合并
            if (!selectedAnnotation.flag) {
                //新建一个标注
                WJAnnotationModel *userAnnotation = [[WJAnnotationModel alloc] init];
    ...
    [annotations enumerateObjectsUsingBlock:^(WJAnnotationModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.dataSource) {
            [obj.dataSource enumerateObjectsUsingBlock:^(WJAnnotationModel * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
                item.flag = NO;
            }];
        }
        else {
    ...
    /*
    //将所有的标注的flag属性恢复为NO，以便下次遍历的时候可继续使用
    for (WJAnnotationModel *annotation in annotations) {
        
        if (annotation.dataSource) {
            for (WJAnnotationModel *item in annotation.dataSource) {
                item.flag = NO;
            }
        }
    }
...
+(NSArray *)calculateMapDataWithArray:(NSArray *)array Distance:(NSNumber *)distance
{
    ...
    for (int i = 0; i < array.count; i++) {
        //数组中有可能存在当前位置（MAUserLocation）或者其他非WJAnnotationModel类的对象
        if ([[array objectAtIndex:i] isKindOfClass:[WJAnnotationModel class]]) {
            WJAnnotationModel *item = [array objectAtIndex:i];
            if ((NSInteger)item.distance < [distance integerValue]) {
                //地图处于缩小状态，比例尺变小
                [annotations addObjectsFromArray:array];
                break;
